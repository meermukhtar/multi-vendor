...
//                                         color: Colors.red, // Assuming green status for accepted orders
//                                       ),
//                                     ),
//                                   ],
//                                 ),
...
//                                 const SizedBox(height: 5.0),
//                                 Text('Price: \$${orderData['price']}'), // Dynamically use price
//                               ],
//                             ),
//                           ),
//                         ],
...
class _RejectedOrderState extends State<RejectedOrder> {
  ...
  List<Map<String, dynamic>> rejectedOrders = []; // Store fetched orders

  @override
  void initState() {
    super.initState();
    // Fetch the rejected orders when the screen loads
    fetchRejectedOrders();
  }

  // Function to fetch rejected orders
  Future<void> fetchRejectedOrders() async {
    User? user = _auth.currentUser;

    if (user != null) {
      try {
        // Get the reference to the "Rejected" sub-collection of the user
        QuerySnapshot ordersSnapshot = await _firestore
            .collection("OrderList")
            .doc(user.uid)
            .collection("Rejected")
        ...
        // Clear the current list of rejected orders
        rejectedOrders.clear();
        if (ordersSnapshot.docs.isEmpty) {
          print('No rejected orders found.');
        }
        // Iterate over the documents in the "Rejected" collection
        for (var doc in ordersSnapshot.docs) {
          Map<String, dynamic> orderData = doc.data() as Map<String, dynamic>;
          rejectedOrders.add(orderData); // Add each order to the list
        }
        ...
        child: SingleChildScrollView(  // To make the content scrollable
          child: Column(
            children: <Widget>[
              ...
              // Loop through the fetched orders and display each in its own Card
                for (var orderData in rejectedOrders)
                  Card(
                    margin: const EdgeInsets.symmetric(vertical: 8.0), // Space between cards
                    elevation: 4.0,  // Card shadow for better visibility
                    child: Padding(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: <Widget>[
                                Row(
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      decoration: const BoxDecoration(
                                        shape: BoxShape.circle,
                                        color: Colors.red, // Red color for rejected orders
                                      ),
                                    ),
                                  ],
                                ),
                                ...
                                Text('Quantity: ${orderData['quantity']}'), // Dynamically use quantity
                                const SizedBox(height: 5.0),
                                Text('Price: ${orderData['price']}'), // Dynamically use price (removed dollar sign)
                              ],
                            ),
                          ),
                        ],
